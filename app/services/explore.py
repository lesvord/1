...
LOCATION_INDEX = {str(index): location for index, location in enumerate(LOCATIONS)}
...
def start_explore(user_id: int, location: str | None = None) -> dict[str, object]:
    target_location = location or LOCATIONS[0]
    if target_location not in LOCATIONS:
        target_location = LOCATIONS[0]
    EXPLORE_STATE[user_id] = ExploreState(current_location=target_location)
...
def handle_callback(data: str, user_id: int) -> dict[str, object] | None:
    ...
    if data.startswith("explore_start_"):
        location_id = data.removeprefix("explore_start_")
        return start_explore(user_id, LOCATION_INDEX.get(location_id))
